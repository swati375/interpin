...
def locatehp(file):
	...
	for key in d1.keys():
		...
		if(ke in gi_all):
			...
			for var in d1[key]:
				v=[]
				st=int(loc_file[indx][1])-(var[0])
				stp=int(loc_file[indx][1])-(var[1])
				v.append(st)
				v.append(stp)
				v.append(var[2])
				v.append(var[3])
	...
	for key in dn.keys():
		sum+=len(dn[key])
		dn[key]=sorted(dn[key],reverse=True)
...
def in_ir(file):
	...
	for key in df.keys():
		for ele in df[key]:
			j=0
			while((ele[1]>int(op[j][2])) and (j<len(op)-1)):
	...
	print('hp in IR rev=',len(data.keys()))
