...
## identify and filter out the hpclusters that lie in interoperonic region
	...
	for i in range(len(st1)):
		for j in range(len(op)):
			if((st1[i]>=int(op[j][1])) and(stp1[i]<=int(op[j][2]))):
				...
				v.append(stp1[i])
				v.append(enr1[i])
				if(st1[i]-int(op[j][1]))<300: # in locationcluster program all hp at <300
					if v not in hpi:
						hpi.append(v)
				else:
					c_up+=1
		...
		for val in d[key]:
			for j in range(len(op)):
				if(val[1]<int(op[j][0])):
					...
					if v not in hpi:
						if(val[0]-int(op[j][1]))<300:  # >300 coming in cases where the stop of gene by ncbi doesnt match operon stop by molquest
							if v not in hpi:
								hpi.append(v)
						else:
							c_up+=1
