...
from functools import reduce # to remove redundant data from list
import re
from operator import itemgetter, attrgetter

#program to form hp clusters and single hp from all hairpins.
...
def hp_dist_new(file):
	di=os.getcwd()
	loc=di+"/../iden_hairpin/"+file+'/forward/'
	...
	for key in d.keys():
		...
		if(c>-1):
			...
			visited=[]
			for var in dn:
				if((var[1][0]-var[0][1])<16) and ((var[1][0]-var[0][1])>0):
			...
			if(len(d_new)>0):
				...
				df_single[key]=sorted(df_single[key])
			elif((len(d_new)==0) and (len(d[key])>0)):
				df_single[key]=sorted(d[key])
			dn=[]
...
#### extracting all mfold hairpins and placing them in dictionary accessed by corresponding sequence number.
	with open(dir+'forward_'+file,'r') as f:
		for line in f:
...
#### sorting hairpins for each gene
	...
	pickle.dump(d_new,open(di+"/../iden_hairpin/"+file+"/forward/d1.p", "wb"))
